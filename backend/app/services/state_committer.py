...
from backend.app.models.models import ActionCheck, AiReview, Character, Clue, GameSession, Message, Task
from backend.app.repositories import (ActionCheckRepo, AiReviewRepo, ClueRepo, FactRepo,
                                      MessageRepo, NpcRepo, ReportRepo, SessionRepo,
                                      TaskRepo)
from backend.app.services.clue_pressure import calc_clue_pressure
from backend.app.services.fact_repository import commit_new_facts, update_npc_alertness
from backend.app.services.rule_service import CheckOutcome
    ...
    *,
    ...
    if check is not None:
        ...
        check_record = ActionCheck(
            ...
            ability_modifier=check.ability_modifier,
    ...
    for task_update in state_updates.task_updates:
        task = (
            db.query(Task)
            .filter(Task.session_id == session.id, Task.title == task_update.title)
            .first()
        )
        if task:
    ...
    if state_updates.scene:
        session.current_scene = state_updates.scene
    if state_updates.summary_delta:
        session.summary = (session.summary or "") + state_updates.summary_delta

    commit_new_facts(
        db,
        session.id,
        state_updates.new_facts,
        current_scene=session.current_scene,
    )
    update_npc_alertness(db, session.id, state_updates.npc_alertness)

    got_key_clue = any(clue.importance == "key" for clue in new_clues)
    session.turns_since_key_clue = 0 if got_key_clue else session.turns_since_key_clue + 1
    failed_checks = (
        db.query(ActionCheck)
        .filter(
            ActionCheck.session_id == session.id,
            ActionCheck.scene == scene_for_check,
            ActionCheck.is_success == 0,
        )
        .count()
    )
    session.clue_pressure = calc_clue_pressure(session.turns_since_key_clue, failed_checks)

    if review is not None:
        scores = review.scores.model_dump() if hasattr(review.scores, "model_dump") else {}
        ai_review = AiReview(
            session_id=session.id,
